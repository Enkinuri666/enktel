...
- **Sports** is brand-led — beIN, Fox Sports, Eurosport, SportKlub, Sportska,
  ESPN, MUTV, Optus Sport, Stan Sport/Events, Paramount+ Sports, Sky Sports,
  TNT, DAZN, SuperSport, Arena Sport, Kayo, Willow and the league names — since
  a generic `/sport/` pattern misses MUTV and DAZN entirely.
...
On the current core set that is 786 of 2,923 — dropping them silently would
delete a quarter of the lineup.
...
retry and auth handling, episode flattening and country matching. Network calls
are stubbed; the suite makes no requests.
